...
public class Funcionario {
    ...
    private Departamento  usuario;

    @Override
    public String toString() {
        return nome + " " + salario+ " " + usuario.getNome();
    }
}
```
...
```java
        ...
        livro.setNome("Lucas");
        livro.setSalario(1000.0);

        //Devemos buscar o usuario da nossa base de dados para aí então atribui-lo a nosso funcionário.
         Departamento usuario = session.createQuery("SELECT d from Departamento d WHERE d.nome=:nome", Departamento.class).setParameter("nome", "Expedição").getSingleResult();

        livro.setDepartamento(usuario);
...
public class DepartamentoDAO {
    ...
    public void save(Departamento usuario) {
        inSession(factory, entityManager -> {
            entityManager.persist(usuario);
        });
    }

    public void delete(Departamento usuario) {
        inSession(factory, entityManager -> {
    ...
    public Departamento findById(int id) {
        inSession(factory, entityManager -> {
            usuario = entityManager.find(Departamento.class, id);
        });
        return usuario;
    }

    public void update(Departamento usuario) {
        inSession(factory, entityManager -> {
...
```java
        ...
        Departamento usuario = new Departamento();
        usuario.setNome("RH");
        //grava o novo usuario no banco de dados
        session.persist(usuario);

        //Criando um usuario utilizando o Builder
        usuario = Departamento.builder().nome("Financeiro").build();
        //grava o novo usuario no banco de dados
        session.persist(usuario);

        //Buscando um Departamento do banco de dados com id=1
        usuario = session.get(Departamento.class,1);
